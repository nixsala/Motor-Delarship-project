...
String rentid = request.getParameter("id");
  try {
	  ...
	  Connection con=DriverManager.getConnection("jdbc:mysql://localhost:3306/motorbike","root","1234");
	  Statement stmt=con.createStatement();
	  ResultSet rs=stmt.executeQuery("select * from bikerent where rentid = '"+rentid+"'");
	  while(rs.next()){
                    ...
                    <form action="update.html" class="contact-form">
                        <div class="row">
                            ...
                            <div class="col-lg-12 text-right">
                                <button type="submit" name = "Submit" formaction="">Drop / Delete</button>
                            </div>
                        </div>
                    </form>
                </div>
...
if(request.getParameterMap().containsKey("Submit")){ //JSP code starting from here 
  ...
  try {
	  ...
	  Connection con=DriverManager.getConnection("jdbc:mysql://localhost:3306/motorbike","root","1234");
	  Statement stmt=con.createStatement();
	  stmt.executeUpdate("DELETE FROM bikerent WHERE  rentermail = '"+rentid+"';");
	  
	  
	  Statement check_connection =con.createStatement();
	  ResultSet rs=check_connection.executeQuery("select * from bikerent where rentermail = '"+rentid+"' ORDER BY rentid DESC LIMIT 1;");
	  boolean val = rs.next(); //next() returns false if there are no-rows retrieved 
      if(val==false){
    	  response.sendRedirect("rides");
       }
      else{
  ...
  }catch(Exception e){ System.out.println(e); response.sendRedirect("logs");}
